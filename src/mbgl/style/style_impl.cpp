...
bool Style::Impl::addClass(const std::string& className) {
    if (hasClass(className)) return false;
    classes.push_back(className);
    return true;
}

bool Style::Impl::hasClass(const std::string& className) const {
    return std::find(classes.begin(), classes.end(), className) != classes.end();
}

bool Style::Impl::removeClass(const std::string& className) {
...
void Style::Impl::setClasses(const std::vector<std::string>& classNames) {
    classes = classNames;
}

std::vector<std::string> Style::Impl::getClasses() const {
    return classes;
}
...
const Image* Style::Impl::getImage(const std::string& id) const {
    return spriteAtlas->getImage(id);
}

void Style::Impl::addImage(const std::string& id, std::unique_ptr<Image> image) {
...
std::vector<std::unique_ptr<Layer>>::const_iterator Style::Impl::findLayer(const std::string& id) const {
    return std::find_if(layers.begin(), layers.end(), [&](const auto& layer) {
        return layer->baseImpl->id == id;
    });
}

Layer* Style::Impl::getLayer(const std::string& id) {
    auto it = findLayer(id);
    return it != layers.end() ? it->get() : nullptr;
}

const Layer* Style::Impl::getLayer(const std::string& id) const {
    auto it = findLayer(id);
    return it != layers.end() ? it->get() : nullptr;
}
...
void Style::Impl::updateTiles(const UpdateParameters& parameters) {
    for (const auto& source : sources) {
...
void Style::Impl::cascade(const TimePoint& timePoint, MapMode mode) {
...
void Style::Impl::recalculate(float z, const TimePoint& timePoint, MapMode mode) {
...
Source* Style::Impl::getSource(const std::string& id) {
...
const Source* Style::Impl::getSource(const std::string& id) const {
    const auto it = std::find_if(sources.begin(), sources.end(), [&](const auto& source) {
        return source->getID() == id;
    });
...
bool Style::Impl::hasTransitions() const {
...
RenderData Style::Impl::getRenderData(MapDebugOptions debugOptions, float angle) const {
    RenderData result;
    ...
    for (const auto& layer : layers) {
        ...
        const Source* source = getSource(layer->baseImpl->source);
        if (!source) {
...
std::vector<Feature> Style::Impl::queryRenderedFeatures(const ScreenLineString& geometry,
...
void Style::Impl::setSourceTileCacheSize(size_t size) {
    for (const auto& source : sources) {
        source->baseImpl->setCacheSize(size);
    }
}

void Style::Impl::onLowMemory() {
    for (const auto& source : sources) {
...
void Style::Impl::onGlyphsLoaded(const FontStack& fontStack, const GlyphRange& glyphRange) {
    observer->onGlyphsLoaded(fontStack, glyphRange);
}

void Style::Impl::onGlyphsError(const FontStack& fontStack, const GlyphRange& glyphRange, std::exception_ptr error) {
    lastError = error;
...
void Style::Impl::onTileChanged(Source& source, const OverscaledTileID& tileID) {
    observer->onTileChanged(source, tileID);
    observer->onUpdate(Update::Repaint);
}

void Style::Impl::onTileError(Source& source, const OverscaledTileID& tileID, std::exception_ptr error) {
    lastError = error;
...
void Style::Impl::onSpriteLoaded() {
...
void Style::Impl::onLayerFilterChanged(Layer& layer) {
    layer.accept(QueueSourceReloadVisitor { updateBatch });
    observer->onUpdate(Update::Layout);
}

void Style::Impl::onLayerVisibilityChanged(Layer& layer) {
    layer.accept(QueueSourceReloadVisitor { updateBatch });
    observer->onUpdate(Update::RecalculateStyle | Update::Layout);
}

void Style::Impl::onLayerPaintPropertyChanged(Layer&) {
    observer->onUpdate(Update::RecalculateStyle | Update::Classes);
}

void Style::Impl::onLayerDataDrivenPaintPropertyChanged(Layer& layer) {
    layer.accept(QueueSourceReloadVisitor { updateBatch });
    observer->onUpdate(Update::RecalculateStyle | Update::Classes | Update::Layout);
}

void Style::Impl::onLayerLayoutPropertyChanged(Layer& layer, const char * property) {
    layer.accept(QueueSourceReloadVisitor { updateBatch });
